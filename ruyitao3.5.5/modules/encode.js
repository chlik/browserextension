...
S.encode = function(table) {
    this.char2unicode_table = table;
    this.unicode2char_table = undefined;
};

S.encode.prototype = {
    getUnicode: function (codepoint) {
        return this.binarySearch(codepoint, this.char2unicode_table);
    },

    getCodepoint: function (unicode) {
        if ( !this.unicode2char_table ) {
            this.buildUnicodeTable();
        }
        return this.binarySearch(unicode, this.unicode2char_table);
    },

    binarySearch: function ( codepoint, table ) {
        var min = 0;
        var max = table.length-1;
        var index = null;
        while ( min <= max ) {
            var mid = Math.floor( (min+max)/2 );
            if ( codepoint < table[mid][0] ) {
                max = mid -1;
            } else if ( codepoint > table[mid][0] ) {
                min = mid + 1;
            } else {
                index = mid;
                break;
        ...
        if ( index === null && max >= 0 ) {
            index = codepoint < table[max][0] ? min : max;
        }
        if ( index != null ) {
            var pair = table[index];
            if ( pair.length == 2 ) {
                return pair[0] == codepoint ? pair[1] : null;
            } else if ( codepoint >= pair[0] && codepoint <= pair[1] ) {
                return pair[2] + ( codepoint - pair[0] );
            }
        }
        return null;
    },
    ...
    buildUnicodeTable: function () {
        var t = [];
        var table = this.char2unicode_table;
        var len = table.length;
        for ( var i=0; i<len; i++ ) {
            var pair = table[i];
            if ( pair.length == 2 ) {
                t.push([pair[1], pair[0]]);
            } else {
                t.push([pair[2], pair[2]+(pair[1]-pair[0]), pair[0]]);
            }
        }
        t.sort(function ( a, b ) { return a[0]-b[0] });
        this.unicode2char_table = t;
    }
};
...
S.encode.gb2312 = {
    ...
    getEncoding: function () {
        if ( typeof this._encoding == "undefined" ) {
            this._encoding = new S.encode(this.table);
        }
        return this._encoding;
    },

    /**
     * convert unicode string to gbk.
     *
     * @param string uni unicode code string
     * @return string gbk encoded string by encodeURI
     */
    encode: function(uni) {
        var to_hex = function ( code ) {
            return '%' + (code > 0x0f ? '' : '0') + code.toString(16).toUpperCase();
        }
        var str = [];
        var len = uni.length;
        for ( var i=0; i<len; i++ ) {
            var c = uni.charCodeAt(i);
            if ( c > 0x80 ) {
                // non-ASCII character
                var codepoint = this.getEncoding().getCodepoint(c);
                if ( codepoint ) {
                    str.push( to_hex((codepoint>>8) + 0x80) );
                    str.push( to_hex((codepoint&0xff) + 0x80) );
                }
            } else {
                str.push( encodeURI(uni.charAt(i)) );
            }
        }
        return str.join('');
    },
    ...
    /**
     * convert gbk to unicode string
     * the input string is encode by encodeURI
     *
     * @param string uri gbk encoded string
    ...
    decode: function(uri) {
        ...
        var str = [];
        var chars = unescape(uri);
        var len = chars.length;
        var pos = 0;
        while ( pos < len ) {
            var first_char = chars.charCodeAt(pos);
            if ( first_char >= 0x80 ) {
                pos++;
                var second_char = chars.charCodeAt(pos);
                var codepoint = ((first_char - 0x80)<<8) + (second_char-0x80);
                var uni = this.getEncoding().getUnicode(codepoint);
                if ( uni ) {
                    str.push( String.fromCharCode(uni) );
                } else {
                    str.push("?");
                }
            } else {
                str.push(chars.charAt(pos));
            }
            pos++;
        }
        return str.join('');
